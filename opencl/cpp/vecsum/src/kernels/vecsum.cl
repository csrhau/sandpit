...
__kernel
void vecsum_vecvecadd(__global const float4 *input_,
                      __global float *output_,
                      __local float4 *scratch_,
                      __const int length_) {
  ...
  if (local_index == 0) {
    output_[get_group_id(0)] = scratch_[0].x + scratch_[0].y 
                             + scratch_[0].z + scratch_[0].w; 
  }
}
...
void vecsum_loadadd(__global const float *input_,
  ...
  if ((global_index + global_size) < length_) {
     scratch_[local_index] += input_[global_index + global_size];
  }
